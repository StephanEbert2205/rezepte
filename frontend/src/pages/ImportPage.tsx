...
import { recipeApi, CreateRecipeData, CreateIngredient } from '../api/client';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const axiosErr = err as {
        response?: { status: number; data?: { error?: string; existingId?: number } };
      };
      if (axiosErr.response?.status === 409) {
        ...
        setStatus('error');
        setMessage(axiosErr.response?.data?.error ?? 'Import fehlgeschlagen. Bitte URL prüfen.');
      }
    }
  };
...
const emptyIng = (): CreateIngredient => ({
  name: '', amount: '', unit: '', optional: false, notes: '',
});
...
function ManualEntry() {
  ...
  const [ingredients, setIngredients]   = useState<CreateIngredient[]>([emptyIng()]);
  ...
  const reset = () => {
    ...
    setIsVegetarian(false); setIsVegan(false);
    setIsGlutenFree(false); setIsLactoseFree(false);
    setTags('');
    ...
    return (
      <div className="card p-6">
        ...
        <div className="flex gap-3">
          <button onClick={() => navigate(`/rezepte/${createdId}`)} className="btn-primary">
            Rezept ansehen
          </button>
          <button onClick={reset} className="btn-secondary">
            Weiteres eingeben
  ...
  // ── Arrays für JSX (außerhalb von JSX definiert, um TS-Parsing-Konflikte zu vermeiden) ──
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="card p-6 space-y-3">
        ...
        <input
          ...
          className="input"
          placeholder="z.B. schnell, pasta, sommer (kommagetrennt)"
        />
      </div>
            ...
            <div key={i} className="space-y-1.5">
              ...
              <div className="flex gap-2 items-center">
                <input
                  type="text"
                  value={ing.amount}
                  onChange={(e) => updIng(i, 'amount', e.target.value)}
              ...
              <div className="flex gap-3 pl-1 items-center">
                ...
                <input
                  type="text"
                  value={ing.notes}
                  onChange={(e) => updIng(i, 'notes', e.target.value)}
          ...
          {instructions.map((content, i) => (
            <div key={i} className="flex gap-2 items-start">
              ...
              <textarea
                value={content}
                onChange={(e) =>
                  setInstructions((p) => p.map((c, j) => (j === i ? e.target.value : c)))
                }
                rows={2}
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-10">
      <div className="mb-6">
        ...
        <p className="mt-1 text-gray-500">Per URL importieren oder selbst eingeben.</p>
      </div>
